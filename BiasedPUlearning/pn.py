...
class Logistic:
    ...
    def pn(self, x, b, t, reg):
        ...
        gp = np.dot(xp, b)
        gn = np.dot(xn, b)
        loss_p = self.loss_func(gp)
        loss_n = self.loss_func(-gn)
        J1 = (self.pi/n1)*np.sum(loss_p)
        J0 = ((1-self.pi)/n0)*np.sum(loss_n)
    ...
    def minimize(self, x, t, reg):
        ...
        func = lambda b: self.pn(x, b, t, reg)
