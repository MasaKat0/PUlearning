...
def train_pu(x_train, t_train, x_test, t_test, pi, epoch, model, optimizer, batchsize=5000, xp=np):
    ...
    for ep in six.moves.range(1, epoch + 1):
        ...
        acc1 = test(x_test, t_test, model, quant=False, xp=np)
        acc2 = test(x_test, t_test, model, quant=True, pi=pi, xp=np)
...
def test(x, t, model, quant=True, pi=False, xp=np):
    X = Variable(xp.array(x, xp.float32))
    theta = 0
    f = model(X).data.T[0]
    if quant is True:
        temp = np.copy(f)
