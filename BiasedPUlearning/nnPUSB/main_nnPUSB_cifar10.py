...
def experiment():
    ...
    for i in range(ite):
        np.random.seed(seed)
        #PN classification
        x_train, t_train, x_test, t_test = load_dataset("cifar10")
        t_train[t_train == -1] = 0
        t_test[t_test == -1] = 0
        ...
        np.savetxt('loss_pu_cifar_%d.csv'%seed, loss_pu, delimiter=',')
        np.savetxt('est_error_pu_cifar_%d.csv'%seed, est_error_pu, delimiter=',')
        np.savetxt('est_error_pubp_cifar_%d.csv'%seed, est_error_pubp, delimiter=',')
        np.savetxt('est_precision_pu_cifar_%d.csv'%seed, est_precision_pu, delimiter=',')
        np.savetxt('est_recall_pu_cifar_%d.csv'%seed, est_recall_pu, delimiter=',')
        np.savetxt('est_precision_pubp_cifar_%d.csv'%seed, est_precision_pubp, delimiter=',')
        np.savetxt('est_recall_pubp_cifar_%d.csv'%seed, est_recall_pubp, delimiter=',')
